module datapath_tb;
    ...
    processor dut(
        ...
        .zero_tb(zero), .forbeq(forbeq), .branch(branch)
    );


    assign ebreak_detected = (instr[6:0] == 7'b1110011) && (instr[20] == 1'b1);

    always @ (posedge clock) begin
        if (ebreak_detected) begin
        $display("EBREAK instruction hit. Test program finished execution!");
        $finish; // Stops the Verilog simulation
        end
    end
